...
const date24h = subHours(Date.now(), 10);
  ...
  return (
    <ctx.Provider
      value={useMemo(
        () => ({
          blockNow,
          block7d: blockTimes[0]?.number
            ? parseInt((blockTimes[0]?.number as unknown) as string)
            : undefined,
          block24h: blockTimes[1]?.number
            ? parseInt((blockTimes[1]?.number as unknown) as string)
            : undefined,
        }),
        [blockNow, blockTimes],
      )}
