...
const fetchReports = async (
  data: MerkleDropClaimsQueryResult['data'],
): Promise<JsonReports> => {
  const promises: Promise<[string, number, JsonReport]>[] = (
    data?.merkleDrops ?? []
  ).flatMap(({ tranches, token: { id } }) =>
    tranches
      .filter(({ claims }) => claims.length === 0)
      .map(async ({ trancheNumber }) => {
        const response = await fetch(
          `/reports/tranches/${trancheNumber}/${id}.json`,
        );
...
export const useMerkleDrops = (
  account?: string,
): { merkleDrops: MerkleDropsMap; refresh(): void } => {
  ...
  return useMemo(() => {
    const merkleDrops =
      account && reports && merkleDropsQuery.data
        ? Object.fromEntries(
            merkleDropsQuery.data.merkleDrops.map(
              ({ id: address, tranches, token }) => {
                const unclaimedTranches = tranches
                  .filter(({ trancheNumber, claims }) => {
                    ...
                    return (
                      claims.length === 0 &&
                      reports[token.id]?.[trancheNumber.toString()]?.[account]
                    );
                  })
                  .map(({ trancheNumber }) => {
