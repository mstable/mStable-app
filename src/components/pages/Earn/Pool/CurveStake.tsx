...
const Input: FC = () => {
  ...
  return (
    <Row>
      <H3 borderTop>Stake LP Token</H3>
      <div>
        <TokenAmountInput
...
const Form: FC = () => {
  ...
  useEffect(() => {
    if (valid && amount && contract) {
      const manifest: SendTxManifest<Interfaces.Gauge, 'deposit(uint256)'> = {
        args: [amount.exact],
  ...
  }, [setFormManifest, valid, amount, contract, curveContracts.musdGauge]);
...
export const CurveStake: FC = () => {
  return (
    <>
      <CurveProtip
        alt="Stake your tokens"
        href="https://www.curve.fi/musd/deposit"
        imgSrc="/media/curve-deposit.gif"
        title="Deposit and stake with Curve for more options"
      >
        <P>
          You can use the{' '}
          <ExternalLink href="https://www.curve.fi/musd/deposit">
            Curve UI
          </ExternalLink>{' '}
          to deposit underlying collateral (mUSD, USDT, USDC and DAI) and stake
          musd3CRV.
        </P>
      </CurveProtip>
