...
const STREAM_TYPES = Object.values(StreamType).filter(
  v => (typeof v as unknown) === 'number',
);
...
export const RewardStreamsProvider: FC<{ refreshInterval?: number }> = ({
  ...
  const rewardStreams = useMemo(() => {
    if (feederPool.vault.account) {
      ...
      const [unlockedStreams, lockedStreams] = rewardEntries
        ...
        .map<Stream[]>(entries =>
          entries.map(entry => {
            ...
            const type =
              start > currentTime ? StreamType.Locked : StreamType.Unlocked;
      ...
      const unlocked = unlockedStreams.reduce(
        (prev, stream) => prev + (stream.amount[StreamType.Unlocked] as number),
        0,
      );
      const locked = lockedStreams.reduce(
      ...
      const idxs = unlockedStreams.length
        ? unlockedStreams.map(stream => stream.index as number).sort()
        : [0, 0]; // Initialisation case
...
export const useRewardStreams = (): RewardStreams | undefined =>
  useContext(ctx);
