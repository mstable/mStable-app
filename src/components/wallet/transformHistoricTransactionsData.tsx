...
export const transformHistoricTransactionsData = (
  ...
  } = Object.fromEntries(Object.values(dataState).map(massetState => [massetState.address, massetState]))
  ...
  return transactions
    .filter(tx => {
      ...
      if (((tx as unknown) as MaybeMassetTx).savingsContract?.masset?.id) {
        return !!massetsByAddress[((tx as unknown) as MaybeMassetTx).savingsContract?.masset?.id]
      }
      return true
    })
    .map(({ hash, id, ...tx }) => {
      ...
      switch (tx.__typename) {
        case 'RedeemTransaction': {
          ...
          const bassetSymbols = tx.bassets.map(basset => (bAssets[basset.id] ?? removedBassets[basset.id])?.token.symbol)
          const massetUnits = new BigDecimal(tx.massetUnits, token.decimals)
          return {
            description: `You redeemed ${massetUnits.format()} ${token.symbol} into ${humanizeList(bassetSymbols)}`,
            hash,
            timestamp,
            formattedDate,
            id,
        ...
        case 'RedeemMassetTransaction': {
          ...
          const massetUnits = new BigDecimal(tx.massetUnits, token.decimals)
          return {
            description: `You redeemed ${massetUnits.format()} ${token.symbol} proportionally into all assets`,
            hash,
            timestamp,
            formattedDate,
            id,
        ...
        case 'MintMultiTransaction': {
          const { bAssets, removedBassets, token } = massetsByAddress[tx.masset.id]
          const bassetSymbols = tx.bassets.map(basset => (bAssets[basset.id] ?? removedBassets[basset.id])?.token.symbol)
          const massetUnits = new BigDecimal(tx.massetUnits, token.decimals)
          return {
            description: `You minted ${massetUnits.simple.toFixed(2)} ${token.symbol} with ${humanizeList(bassetSymbols)}`,
            hash,
            timestamp,
            formattedDate,
            id,
        ...
        case 'MintSingleTransaction': {
          const { bAssets, removedBassets, token } = massetsByAddress[tx.masset.id]
          const massetUnits = new BigDecimal(tx.massetUnits, token.decimals)
          const bassetSymbol = (bAssets[tx.basset.id] ?? removedBassets[tx.basset.id])?.token.symbol
          return {
            description: `You minted ${massetUnits.format()} ${token.symbol} with ${bassetSymbol}`,
            hash,
            timestamp,
            formattedDate,
            id,
        ...
        case 'PaidFeeTransaction': {
          ...
          const bassetUnit = new BigDecimal(tx.bassetUnits, basset.token.decimals)
          return {
            description: `You paid ${bassetUnit.format()}${basset ? ` ${basset.token.symbol}` : ''} in fees`,
            hash,
            timestamp,
            formattedDate,
            id,
        ...
        case 'SavingsContractDepositTransaction': {
          ...
          return {
            description: `You deposited ${massetUnits.format()} ${token.symbol} into SAVE`,
            hash,
            timestamp,
            formattedDate,
            id,
        ...
        case 'SavingsContractWithdrawTransaction': {
          ...
          const massetUnits = new BigDecimal(tx.amount, token.decimals)
          return {
            description: `You withdrew ${massetUnits.format()} ${token.symbol} from SAVE`,
            hash,
            timestamp,
            formattedDate,
            id,
        ...
        case 'SwapTransaction': {
          const { bAssets, removedBassets, token } = massetsByAddress[tx.masset.id]
          const inputBasset = (bAssets[tx.inputBasset.id] ?? removedBassets[tx.inputBasset.id])?.token.symbol
          const outputBasset = (bAssets[tx.outputBasset.id] ?? removedBassets[tx.outputBasset.id])?.token.symbol
          const massetUnits = new BigDecimal(tx.massetUnits, token.decimals)
          return {
    ...
    .filter(Boolean)
    .sort((a, b) => (b as HistoricTransaction).timestamp - (a as HistoricTransaction).timestamp) as HistoricTransaction[]
}
